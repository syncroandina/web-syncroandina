        ...
        <div class="max-w-6xl mx-auto bg-white/70 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/50 overflow-hidden flex flex-col md:flex-row animate-fade-in-up">
            ...
            <div class="md:w-3/5 p-12 lg:p-16">
                ...
                <form id="contact-interactive-form" class="space-y-6">
                    ...
                    <div class="space-y-3 hidden transform origin-top transition-all duration-300 scale-95 opacity-0" id="interest-product-selector-wrapper">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <!-- Del Catálogo -->
                            <label class="relative flex items-center justify-between p-4 bg-gray-50 border-2 border-secondary/80 rounded-2xl cursor-pointer hover:bg-white transition-all shadow-sm group" id="product-type-catalog-lbl">
                                <span class="flex items-center gap-3">
                                    <span class="p-2.5 bg-blue-50 text-secondary rounded-xl group-hover:bg-blue-100 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path></svg>
                                    </span>
                                    <span class="font-bold text-gray-800 text-sm">Elegir del catálogo</span>
                                </span>
                                <span class="flex items-center h-5">
                                    <input type="radio" name="product_selection_type" value="catalog" checked class="text-secondary focus:ring-secondary w-4 h-4 cursor-pointer m-0">
                                </span>
                            </label>
                            
                            <!-- Manual (Otro) -->
                            <label class="relative flex items-center justify-between p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl cursor-pointer hover:bg-white hover:border-secondary/30 transition-all shadow-sm group" id="product-type-manual-lbl">
                                <span class="flex items-center gap-3">
                                    <span class="p-2.5 bg-blue-50 text-secondary rounded-xl group-hover:bg-blue-100 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </span>
                                    <span class="font-bold text-gray-800 text-sm">Escribir manualmente</span>
                                </span>
                                <span class="flex items-center h-5">
                                    <input type="radio" name="product_selection_type" value="manual" class="text-secondary focus:ring-secondary w-4 h-4 cursor-pointer m-0">
                                </span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 hidden transform origin-top transition-all duration-300 scale-95 opacity-0" id="interest-product-custom-wrapper">
                        <label class="text-sm font-semibold text-gray-700">Especificar Repuesto / Componente <span class="text-red-500">*</span></label>
                        <input type="text" name="custom_product" id="product-custom-input" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-secondary focus:border-transparent transition-all shadow-sm" placeholder="Ej. Filtro de aire personalizado">
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const productSelectorWrapper = document.getElementById('interest-product-selector-wrapper');
    const productWrapper = document.getElementById('interest-product-wrapper');
    const customProductWrapper = document.getElementById('interest-product-custom-wrapper');
    ...
    const customProductInput = document.getElementById('product-custom-input');

    function toggleProductSelectionType(selectionType) {
        const catalogLabel = document.getElementById('product-type-catalog-lbl');
        const manualLabel = document.getElementById('product-type-manual-lbl');
    ...
    function toggleInterestFields(type) {
        ...
        setTimeout(() => {
            if (type === 'servicio' && serviceWrapper && serviceSelect) {
                ...
                const selectedRadio = document.querySelector('#contact-interactive-form input[name="product_selection_type"]:checked');
                const selectionType = selectedRadio ? selectedRadio.value : 'catalog';
                toggleProductSelectionType(selectionType);
            }
        }, 160);
    }
    ...
    const productSelectionRadios = document.querySelectorAll('#contact-interactive-form input[name="product_selection_type"]');
    productSelectionRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            toggleProductSelectionType(e.target.value);
        });
    });
